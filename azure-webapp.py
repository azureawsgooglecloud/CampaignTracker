...
connection_string = "Driver={ODBC Driver 13 for SQL Server};Server=tcp:2434334.database.windows.net,1433;Database=243242;Uid=2434242;Pwd={2422323@$1Bj5};Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;"
app = Flask(__name__)
...
def home():
	if request.method == 'POST':
		...
		try:
			query_string = """INSERT INTO Optimove.CampaignTracker ( CAMPAIGNNAME, CAMPAIGNDETAILS, CAMPAIGNSTARTDATE, CAMPAIGNENDDATE, CAMPAIGN_ID, CAMPAIGNBRANDNAME, CAMPAIGNCHANNEL, CAMPAGINTYPE, CAMPAGINFILENAME) VALUES('{}','{}','{}','{}','{}', '{}','{}','{}','{}')""".format(data['campaign_name'],data['campaign_details'],data['campaign_start'],data['campaign_end'],data['campaign_uniqueId'], data['campaign_type'], data['campaign_brand_name'], data['campaign_channel_type'], filename)
			# print(query_string)
...
# Not tested or working at this stage
...
# Not tested or working at this stage. Insert PlayerID into DB table 
def saveDataFrameToDB(db,listObj, id, uniqueId):		
	try:
		if id > 0:
			sql_statement = 'INSERT INTO Optimove.campaignTrackerPlayerDetails (campaign_id, Value) VALUES ({},?)'.format(id)			
			cursor = db.cursor()
			...
			if len(listObj) != getInsertedDetailsRowCount(db,id):			
				return { 'error': 'Failed to save all data rows to DB - 03', 'statusCode': '400'}						
...
def getInsertedDetailsRowCount(db, id):	
	try:
		sql_statement = 'SELECT Count(id) FROM Optimove.campaignTrackerPlayerDetails WHERE campaign_id = {}'.format(id)
		cursor = db.cursor()
